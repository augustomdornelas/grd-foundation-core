// ============================================================
// /app/colaboradores — a lista e a ficha de quem já está dentro
// ------------------------------------------------------------
// É o único cadastro de gente do Portal. Colaborador entra por aqui de
// três jeitos: admissão concluída, importação da Secullum (aba ao
// lado) ou o botão "Novo colaborador". Não sai por exclusão: sai pelo
// desligamento, que guarda data, motivo e histórico.
//
...
import { Plus, Search, FileDown, HardHat, RefreshCw, Trash2, Pencil } from "lucide-react";
...
export const Route = createFileRoute("/app/colaboradores/")({ component: RhColaboradores });
...
function ColaboradorFicha({ id, onFechar }: { id: string | null; onFechar: () => void }) {
  ...
  return (
    <>
      ...
      {editando && <ColaboradorFormDialog colaborador={col} onClose={() => setEditando(false)} />}
